  ...
  > _
    ...
    Private Sub btnProcess_Click(sender As System.Object, e As System.EventArgs) Handles btnProcess.Click
        ...
        Try
            ...
            If PassedFromXSVCUPDT = False Then
                Call Status(StartProcess, False, "", LOG_AND_DISP)
            End If
            ...
            Do While serr1 = 0
                ...
                'Call Status(SaveGoodArgVals, False, "Contract:" + SParm(Contractid.Trim) + "SQL:" + SParm(SQLstmt.Trim), LOG_AND_DISP)
                ...
                Else
                    ...
                    If DateCmp(bMassUp.EffectDate, CurrDate) < 0 Or DateCmp(bMassUp.EffectDate, CurrDate) = 0 Then
                        ...
                        Call Status(SaveGoodArgVals, False, "Call Cancel Contract SQL:" + SParm(Contractid.Trim), LOG_AND_DISP)
                        Call CancelContract(Contractid.Trim)
        ...
        Catch ex As Exception
            MessageBox.Show("Error: " + ex.Message.ToString.Trim)
        End Try

        If PassedFromXSVCUPDT = False Then
            Call Status(EndProcess, False, "", LOG_AND_DISP)
        Else
            Call ScreenExit("", "")
        End If
    End Sub
    ...
    Private Sub CancelContract(ByVal CancContractID As String)
        Dim cancelCode As String
        Dim sysDate As Integer
        Dim csr_cancel As Integer

        Call SqlCursor(csr_cancel, NOLEVEL)

        cancelCode = "14"
        Call GetSysDate(sysDate)

        'Call MsgBox("UPDATING CONTRACT")
        SQLstmt = "XSNCANC_UpdateSMcontract" + SParm(CancContractID) + SParm(cancelCode) + SParm(bpes.UserId)
        Call Status(SaveGoodArgVals, False, "Contract:" + SParm(CancContractID.Trim) + " SQL:" + SParm(SQLstmt.Trim), LOG_AND_DISP)
        'Call MsgBox("CONTRACT UPDATE COMPLETE")
        Call sql(csr_cancel, SQLstmt)

        'INSERT SERVICES CANCEL HERE
        SQLstmt = "XSNCANC_CancelServices" + SParm(CancContractID) + SParm(bpes.UserId)
        Call Status(SaveGoodArgVals, False, "Contract:" + SParm(CancContractID.Trim) + " SQL:" + SParm(SQLstmt.Trim), LOG_AND_DISP)
        Call sql(csr_cancel, SQLstmt)

        SQLstmt = "XDelete_AllContractEquip_Fnl" + SParm(CancContractID) + DParm(sysDate)
        Call Status(SaveGoodArgVals, False, "Contract:" + SParm(CancContractID.Trim) + " SQL:" + SParm(SQLstmt.Trim), LOG_AND_DISP)
        Call sql(csr_cancel, SQLstmt)

        Call SqlFree(csr_cancel)
